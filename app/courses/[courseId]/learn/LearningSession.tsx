...
export default function LearningSession({ userId, courseId, sessionLength, firstWord }: LearningSessionProps) {
  const [progress, setProgress] = useState(0);
  const [correctAnswers, setCorrectAnswers] = useState(0);
  const [currentWord, setCurrentWord] = useState(firstWord);
  const [currentStep, setCurrentStep] = useState(firstWord.show_first_step ? 1 : firstWord.step);
  ...
  const renderCurrentStep = () => {
    ...
    console.log(`Rendering step ${currentStep} for word: ${currentWord.words.word}`);

    // Check if show_first_step is true and render step 1
    if (currentWord.show_first_step) {
      return <Step1 word={currentWord} onNext={handleNextStep} setCorrectAnswers={setCorrectAnswers} />;
    }
    ...
    switch (currentStep) {
      ...
      case 2:
        return (
          <Step2 word={currentWord} onNext={handleNextStep} courseId={courseId} setCorrectAnswers={setCorrectAnswers} />
        );
      case 3:
        return (
          <Step3 word={currentWord} onNext={handleNextStep} courseId={courseId} setCorrectAnswers={setCorrectAnswers} />
        );
      case 4:
        return (
          <Step4 word={currentWord} onNext={handleNextStep} courseId={courseId} setCorrectAnswers={setCorrectAnswers} />
        );
      case 5:
        return (
          <Step5 word={currentWord} onNext={handleNextStep} courseId={courseId} setCorrectAnswers={setCorrectAnswers} />
        );
      case 6:
        return (
          <Step6 word={currentWord} onNext={handleNextStep} courseId={courseId} setCorrectAnswers={setCorrectAnswers} />
        );
      case 7:
        return <Step7 word={currentWord} onNext={handleNextStep} setCorrectAnswers={setCorrectAnswers} />;
      case 8:
        return <Step8 word={currentWord} onNext={handleNextStep} setCorrectAnswers={setCorrectAnswers} />;
      case 9:
        return <Step9 word={currentWord} onNext={handleNextStep} setCorrectAnswers={setCorrectAnswers} />;
  ...
  return (
    <div className='max-w-3xl mx-auto p-6 bg-white rounded-lg space-y-6'>
      ...
      />
      <div className='text-right text-gray-500'>{Math.round(progress)}% completed</div>
      {renderCurrentStep()}
    </div>
  );
}
