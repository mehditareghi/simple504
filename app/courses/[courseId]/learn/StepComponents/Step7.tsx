...
const Step7: FC<Step7Props> = ({ word, onNext, setCorrectAnswers }) => {
  ...
  const onSubmit = async () => {
    const correct = transcript.toLowerCase().trim() === word.words.word.toLowerCase();
    let updatePattern = {};
    if (correct) {
  ...
  return (
    <Card className='p-6 bg-white rounded-lg shadow-lg space-y-4'>
      <CardHeader className='mb-4'>
        <CardTitle className='text-2xl font-bold text-gray-800'>Pronounce the word: {word.words.word}</CardTitle>
      ...
      <CardContent>
        ...
        <p className='mt-4'>
          Spoken word:{' '}
          <span className={`font-bold ${isCorrect === false ? 'text-red-500' : 'text-green-500'}`}>
            {transcript.toLowerCase().trim()}
          </span>
        </p>
        <Separator className='my-4' />
        <Button onClick={submitted ? onNext : onSubmit} className='w-full' disabled={listening || !transcript}>
