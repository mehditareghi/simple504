...
export async function POST(req: Request) {
  try {
    ...
    const { userId, maxWords = 10, newWordsCount = 3, targetCorrectAnswers = 10 } = await req.json();
    ...
    if (newWordsError) {
      ...
      return NextResponse.json({ error: newWordsError.message }, { status: 400 });
    }
    ...
    if (partialWordsError) {
      console.error('Error fetching partially learned words:', partialWordsError);
      return NextResponse.json({ error: partialWordsError.message }, { status: 400 });
    }

    console.log('Partially learned words fetched:', partialWords);

    const sessionWords = [
      ...newWords.map((word) => ({ session_id: sessionId, word_id: word.word_id, current_step: word.step })),
      ...partialWords.map((word) => ({ session_id: sessionId, word_id: word.word_id, current_step: word.step })),
    ];
    ...
    return NextResponse.json({ sessionId, targetCorrectAnswers, words: detailedWords });
  } catch (error) {
